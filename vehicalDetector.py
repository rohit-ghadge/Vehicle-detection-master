...
# Applying on each frame
    ...
    cv2.line(video, (25,count_line_position),(1200,count_line_position),(255,0,255), 3)
    ...
    for (i, c) in enumerate(countersahpe):
        ...
        cv2.putText(video,"Vehicle Count: " + str(counter), (x,y-20),cv2.FONT_HERSHEY_TRIPLEX,1,(255,244,0),2)


        center = center_handle(x,y,w,h)
        detect.append(center)
        cv2.circle(video, center, 4, (255,0,0), -1)

        for (x,y) in detect:
            if y<(count_line_position + offset) and  y>(count_line_position - offset):
                counter+=1
                cv2.line(video, (25,count_line_position),(1200,count_line_position),(0,0,255), 3)
                detect.remove((x,y))

                print("Vehicle Count: "+ str(counter))

    cv2.putText(video,"Detected Vehical Count: " + str(counter), (800,50),cv2.FONT_HERSHEY_COMPLEX,1,(147,55,255),2)
